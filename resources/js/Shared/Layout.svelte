  ...
  <div class="md:flex md:h-screen md:flex-col">
    <div class="md:flex md:shrink-0">
      ...
      <div class="md:text-md flex w-full items-center justify-between border-b bg-white p-4 text-sm md:px-12 md:py-0">
        <div class="mr-4 mt-1 text-lg font-bold">Agenda Penomoran Surat Keluar Otomatis</div>
        {#if auth && auth.user}
        ...
        {:else}
        <a use:inertia href="/login" class="text-indigo-600 hover:underline">
          Login
        </a>
        {/if}
      </div>
    ...
    <div class="md:flex md:grow md:overflow-hidden">
      <MainMenu class="hidden w-56 shrink-0 overflow-y-auto bg-indigo-800 p-4 md:block" />
      <div class="px-4 md:flex-1 md:overflow-y-auto md:px-12 py-6" scroll-region>
        <FlashMessages />
        <slot />
      </div>
    </div>
