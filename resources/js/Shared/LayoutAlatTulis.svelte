...
<script>
  ...
  import MainMenuAlatTulis from '@/Shared/MainMenuAlatTulis.svelte'

  export let auth
</script>

<svelte:head>
  <title>{$title ? `${$title} - BPS Kab Magelang` : 'BPS'}</title>
</svelte:head>
    ...
    <div class="md:flex md:shrink-0">
      <div class="flex items-center justify-between bg-indigo-900 px-6 py-4 md:w-56 md:shrink-0 md:justify-center">
        ...
        <Dropdown class="md:hidden" placement="bottom-end">
          ...
          <div slot="dropdown" class="mt-2 rounded bg-indigo-800 px-4 py-4 shadow-lg">
            <MainMenuAlatTulis />
          </div>
        </Dropdown>
      </div>
      <div class="md:text-md flex w-full items-center justify-between border-b bg-white p-4 text-sm md:px-12 md:py-0">
        <div class="mr-4 mt-1 text-lg font-bold">Permintaan Alat Tulis Kantor</div>
        {#if auth && auth.user}
        ...
        {:else}
          <a use:inertia href="/login" class="rounded bg-indigo-600 px-4 py-2 font-bold text-white transition hover:bg-indigo-700"> Masuk </a>
        {/if}
      </div>
    </div>
    <div class="md:flex md:grow md:overflow-hidden">
      <MainMenuAlatTulis class="hidden w-56 shrink-0 overflow-y-auto bg-indigo-800 p-4 md:block" />
      <div class="px-4 py-6 md:flex-1 md:overflow-y-auto md:px-12" scroll-region>
        <FlashMessages />
        <slot />
      </div>
    </div>
