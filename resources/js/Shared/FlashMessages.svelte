<script>
  import { page } from '@inertiajs/svelte'

  let show = true
  let timeout

  $: if ($page.props.flash || Object.keys($page.props.errors).length > 0) {
    show = true
    clearTimeout(timeout)
    timeout = setTimeout(() => {
      show = false
    }, 4000)
  }

  function close() {
    show = false
  }
</script>

<div class="pointer-events-none fixed bottom-6 right-6 z-50 flex flex-col gap-3">
  {#if $page.props.flash.success && show && !$page.props.flash.success.includes('Nomor Surat') && $page.props.flash.success !== 'Surat berhasil diupdate!'}
    <div class="pointer-events-auto flex min-w-[300px] items-center justify-between rounded bg-green-500 shadow-lg">
      <div class="flex items-center">
        <svg xmlns="http://www.w3.org/2000/svg" class="ml-4 mr-2 h-5 w-5 shrink-0 fill-white" viewBox="0 0 20 20" fill="currentColor">
          <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
        </svg>
        <div class="py-3 pr-4 text-sm font-medium text-white">{$page.props.flash.success}</div>
      </div>
      <button type="button" class="group mr-2 p-2" on:click={close}>
        <svg class="block h-2 w-2 fill-green-800 group-hover:fill-white" width="235.908" height="235.908" viewBox="278.046 126.846 235.908 235.908">
          <path
            d="M506.784 134.017c-9.56-9.56-25.06-9.56-34.62 0L396
            210.18l-76.164-76.164c-9.56-9.56-25.06-9.56-34.62 0-9.56 9.56-9.56 25.06 0 34.62L361.38
            244.8l-76.164 76.165c-9.56 9.56-9.56 25.06 0 34.62 9.56 9.56 25.06 9.56 34.62 0L396
            279.42l76.164 76.165c9.56 9.56 25.06 9.56 34.62 0 9.56-9.56 9.56-25.06 0-34.62L430.62
            244.8l76.164-76.163c9.56-9.56 9.56-25.06 0-34.62z" />
        </svg>
      </button>
    </div>
  {/if}

  {#if $page.props.flash.error || (Object.keys($page.props.errors).length > 0 && show)}
    <div class="pointer-events-auto flex min-w-[300px] items-center justify-between rounded bg-red-500 shadow-lg">
      <div class="flex items-center">
        <svg class="ml-4 mr-2 h-4 w-4 shrink-0 fill-white" viewBox="0 0 20 20">
          <path
            d="M2.93 17.07A10 10 0 1 1 17.07 2.93 10 10 0 0 1 2.93 17.07zm1.41-1.41A8 8 0 1 0 15.66
            4.34 8 8 0 0 0 4.34 15.66zm9.9-8.49L11.41 10l2.83 2.83-1.41 1.41L10 11.41l-2.83
            2.83-1.41-1.41L8.59 10 5.76 7.17l1.41-1.41L10 8.59l2.83-2.83 1.41 1.41z" />
        </svg>
        {#if $page.props.flash.error}
          <div class="py-3 pr-4 text-sm font-medium text-white">{$page.props.flash.error}</div>
        {:else}
          <div class="py-3 pr-4 text-sm font-medium text-white">
            {#if Object.keys($page.props.errors).length === 1}
              <span>Terdapat isian yang tidak sesuai</span>
            {:else}
              <span>There are {Object.keys($page.props.errors).length} form errors.</span>
            {/if}
          </div>
        {/if}
      </div>
      <button type="button" class="group mr-2 p-2" on:click={close}>
        <svg class="block h-2 w-2 fill-red-800 group-hover:fill-white" width="235.908" height="235.908" viewBox="278.046 126.846 235.908 235.908">
          <path
            d="M506.784 134.017c-9.56-9.56-25.06-9.56-34.62 0L396
            210.18l-76.164-76.164c-9.56-9.56-25.06-9.56-34.62 0-9.56 9.56-9.56 25.06 0 34.62L361.38
            244.8l-76.164 76.165c-9.56 9.56-9.56 25.06 0 34.62 9.56 9.56 25.06 9.56 34.62 0L396
            279.42l76.164 76.165c9.56 9.56 25.06 9.56 34.62 0 9.56-9.56 9.56-25.06 0-34.62L430.62
            244.8l76.164-76.163c9.56-9.56 9.56-25.06 0-34.62z" />
        </svg>
      </button>
    </div>
  {/if}
</div>
